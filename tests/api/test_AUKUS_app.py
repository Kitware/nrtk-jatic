...
from nrtk_cdao.api.schema import NrtkPybsmPerturbOutputSchema, DatasetSchema
from nrtk_cdao.api.aukus_app import AUKUS_app, Settings
from nrtk_cdao.api.aukus_schema import AukusDatasetSchema
from typing import Generator
from tests import DATASET_FOLDER, NRTK_CONFIG, BAD_NRTK_CONFIG
...
def test_handle_aukus_post(
    ...
    responses.add(method="POST",
                  ...
                  json=jsonable_encoder(NrtkPybsmPerturbOutputSchema(
...
def test_bad_nrtk_config_post(test_aukus_client: TestClient,
                              tmpdir: py.path.local) -> None:
    Aukus_Dataset = AukusDatasetSchema(
                        docType="Dataset Metadata",
                        docVersion="0.1",
                        ...
                        dataCollections=[],
                        dataFormat="YOLO",
                        nrtkConfig=str(BAD_NRTK_CONFIG),
                        outputDir=str(tmpdir),
                        labels=[
                            {
                                "name": "AUKUS",
...
def test_bad_NRTK_config_post(test_aukus_client: TestClient,
    ...
    assert response.status_code == 400
    assert response.json()['detail'] == "NRTK config missing gsds parameter."


def test_bad_NRTK_config_post_2(test_aukus_client: TestClient,
                                tmpdir: py.path.local) -> None:
    Aukus_Dataset = AukusDatasetSchema(
                        docType="Dataset Metadata",
                        docVersion="0.1",
