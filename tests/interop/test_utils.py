...
@pytest.mark.parametrize("dataset, img_filenames, categories, expectation", [
    (
        ...
        ["images/img1.png"],
        [{"id": 0, "name": "cat0", "supercategory": None}, {"id": 1, "name": "cat1", "supercategory": None}],
        does_not_raise()
    ),
    (
        JATICObjectDetectionDataset(
...
def test_dataset_to_coco(
    ...
    with expectation:
        ...
        label_file = tmppath / "annotations.json"
        assert label_file.is_file()
        metadata_file = tmppath / "image_metadata.json"
        assert metadata_file.is_file()

        # Confirm images exist
        img_paths = [tmppath / filename for filename in img_filenames]
