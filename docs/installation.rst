...
There are two ways to obtain the nrtk-jatic package.
...
.. prompt:: bash
    ...
    git clone https://gitlab.jatic.net/jatic/kitware/nrtk-jatic ./
    poetry install
    poetry run pytest
    cd docs
...
^^^^^^^^^^^^^^^^^^^^^^^^^^
The documentation for nrtk-jatic is maintained as a collection of
...
as well as the source files in :file:`src/nrtk_jatic/`, for changes.
